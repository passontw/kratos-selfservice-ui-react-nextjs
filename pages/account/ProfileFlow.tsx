...
import LoginVerificationCheckbox from "./LoginVerificationCheckbox"
...
> {
  ...
  render() {
    ...
    const csrfTokenNode = nodes.find(node => node.attributes.name === "csrf_token");
    const loginVerificationNode = nodes.find(node => node.attributes.name === "traits.loginVerification")
    const submitNode = nodes.find(node => node.attributes.name === "method" && node.attributes.type === "submit")
    ...
    return (
      ...
      >
        ...
        />
        <LoginVerificationCheckbox
          dispatchSubmit={this.handleSubmit}
          value={values[getNodeId(loginVerificationNode)]}
          setValue={(value) =>
            new Promise((resolve) => {
              this.setState(
                (state) => ({
                  ...
                  values: {
                    ...state.values,
                    [getNodeId(loginVerificationNode)]: value,
                  },
                }),
                resolve,
              )
          ...
          node={loginVerificationNode}
          attributes={loginVerificationNode.attributes}
        />
        <NodeInputSubmit
